...
SparkFun_LPS33HW_Arduino_Library.cpp

Header file: SparkFun_LPS33HW_Arduino_Library.h
...
/**
   * Constructor for an object of the LPS33HW class
*/
LPS33HW::LPS33HW( void )
{
...
*/
bool LPS33HW::begin( TwoWire &wirePort, uint8_t address, uint32_t clock_frequency)
{
	...
	values[0] = LPS33HW_RES_CONF_DEFAULT;									// Set the resolution configuration to default
	write( LPS33HW_REG_RES_CONF, values, 1 );
																			// There is a break here because the RES_CONF register is not contiguous with the other registers
	values[0] = LPS33HW_CTRL_REG1_ODR_25HZ ;	                            // Turn the sensor ON and set output rate to 25 Hz
	values[1] = LPS33HW_CTRL_REG2_DEFAULT;									// Default
	values[2] = LPS33HW_CTRL_REG3_INT_L | LPS33HW_CTRL_REG3_OD ;			// Set interrupts to output LOW and Open Drain function
	write( LPS33HW_REG_CTRL_REG1, values, 3 );								// Write the 5 values consecutively to the device using only one write function
    
    values[0] = LPS33HW_INTERRUPT_CFG_DEFFAULT;                             // Set the Interrupt CFG register to default
    write( LPS33HW_REG_RES_CONF, values, 1 );
...
/**
   * Determines if a LPS33HW device is available at the address associated with the object
   * @return Boolean, true if connected and false if disconnected
*/
bool LPS33HW::isConnected()
{
	_i2cPort->beginTransmission(sensor_address);							// Starting an I2C transmission will allow us to check for acknowledgement by any device at 'sensor_address'
  	...
  	{
  		if(getID() == LPS33HW_DEVID)										// If there was a response then we can make sure the device is in fact a LPS33HW by checking the ID
  		{
  			lastCode = LPS33HW_CODE_CONNECTED;
  			return true;
  		}
  	...
  	lastCode = LPS33HW_CODE_NO_DEV_AT_ADDRESS;
  	return false;															
}

/**
   * Gets the device ID from the WHO_AM_I register. The return should be equal to 0xBD for valid LPS33HW devices
   * @return Device ID
*/
uint8_t LPS33HW::getID()
{
	uint8_t retval = 0x00;											
	read( LPS33HW_REG_WHO_AM_I, &retval, 1 );		// Use the read function to read the WHO_AM_I register into the 'retval' variable
...
/**
   ...
   * @return Status code
*/
uint8_t LPS33HW::getStatus()
{
	uint8_t retval = 0x00;							
	read( LPS33HW_REG_STATUS_REG, &retval, 1 );		// Use the read function to read the STATUS_REG register into the 'retval' variable
	return retval;									// Then return the 'retval' variable
}
...
*/
int16_t		LPS33HW::getTemperature_raw()
{
	uint8_t data[2];								// Allocate 2 bytes to hold the 16 bit data
	read(LPS33HW_REG_TEMP_OUT_L, data, 2 );			// Use the read function to fill out both those bytes from first the TEMP_OUT_L and then the TEMP_OUT_H registers
...
/**
   ...
   * @return Temperature reading
*/
float		LPS33HW::getTemperature_degC()
{
	...
	/*
	NOTE! 	The LPS33HW datasheet does not specify the 42.5 deg C offset,
...
*/
int32_t 	LPS33HW::getPressure_raw()
{
	uint8_t data[3];																			// Allocate 3 bytes for the 24 bit data
	read(LPS33HW_REG_PRESS_OUT_XL, data, 3 );													// Read all 3 bytes in order beginning with the extra low byte
...
*/
bool LPS33HW::setReferencePressure(uint32_t adc_val)
{
	...
	write( LPS33HW_REG_REF_P_XL, data, 3 );		// Write the data to the device beginning witht the LSB (least significant byte)
}
...
/**
   ...
   * @return Boolean, true if successful and false if unsuccessful
*/
bool LPS33HW::setPressureThreshold(uint16_t adc_val)
{
	...
	write( LPS33HW_REG_THS_P_L, data, 2 );
}
...
/**
   ...
   * @return Boolean, true if successful and false if unsuccessful
*/
bool LPS33HW::setTemperatureAverages(uint8_t avg_code)
{
	return applySetting(LPS33HW_REG_RES_CONF, (avg_code & 0x0C));
}
...
/**
   ...
   * @return Boolean, true if successful and false if unsuccessful
*/
bool LPS33HW::setPressureAverages(uint8_t avg_code)
{
	return applySetting(LPS33HW_REG_RES_CONF, (avg_code & 0x03));
}
...
/**
   ...
   * @return Boolean, true if successful and false if unsuccessful
*/
bool LPS33HW::setOutputDataRate(uint8_t odr_code)
{
	return applySetting(LPS33HW_REG_CTRL_REG1, (odr_code & 0x70));
}
...
/**
   ...
   * @return Boolean, true if successful and false if unsuccessful
*/
bool LPS33HW::setFIFOMode(uint8_t mode_code)
{
	if(mode_code == LPS33HW_FIFO_CTRL_BYPASS)	// If we want to bypass the FIFO then we should disable the FIFO
	{
		// Using 'removeSetting()' along with FIFO_EN will 'reset' (means set to 0) the FIFO enable bit
		if(removeSetting(LPS33HW_REG_CTRL_REG2, LPS33HW_CTRL_REG2_FIFO_EN) != true)
		{
			return false;
		}
	}
	else
	{
		...
		if(applySetting(LPS33HW_REG_CTRL_REG2, LPS33HW_CTRL_REG2_FIFO_EN) != true)
		{
			return false;
		}
	}
	...
	if(applySetting(LPS33HW_REG_FIFO_CTRL, (mode_code & 0xE0) ) != LPS33HW_CODE_NOM)
	{
		return false;
	}		
...
/**
   ...
   * @return Boolean, true if successful and false if unsuccessful
*/
bool LPS33HW::setFIFOMeanNum(uint8_t num_code)
{
	return applySetting(LPS33HW_REG_FIFO_CTRL, (num_code & 0x1F));
}
...
/**
   ...
   * @param reg_adr The address of the register to modify. Use of the LPS33HW_RegistersTypeDef enumeration values is suggested.
   ...
   * @see LPS33HW_RegistersTypeDef
   * @see Defined setting values in .h file
   * @return Boolean, true if successful and false if unsuccessful
*/
bool LPS33HW::applySetting(uint8_t reg_adr, uint8_t setting)
{
	...
	if(write(reg_adr, &data, 1 ) != LPS33HW_CODE_NOM)							// Write the new data back to the device and make sure it was successful
	{
		lastCode = LPS33HW_CODE_SET_FAIL;
		return false;															// If it failed then return an error
	}
	lastCode = LPS33HW_CODE_NOM;
...
*/
bool LPS33HW::removeSetting(uint8_t reg_adr, uint8_t setting)
{
	uint8_t data;																// Declare space for the data
	read(reg_adr, &data, 1 );													// Now fill that space with the old setting from the sensor
	...
	if(write(reg_adr, &data, 1 ) != LPS33HW_CODE_NOM)							// Write the new data back to the device and make sure it was successful
	{
		lastCode = LPS33HW_CODE_RESET_FAIL;
		return false;															// If it failed then return an error
	}
	lastCode = LPS33HW_CODE_NOM;
	return true;																// Otherwise its all good!
...
/**
   ...
   * @param reg_adr The address of the first register to read from. Use of the LPS33HW_RegistersTypeDef enumeration values is suggested.
   ...
   * @return Boolean, true if read was successful and false if unsuccessful
*/
bool LPS33HW::read( uint8_t reg_adr, uint8_t * pdata, uint8_t size)
{
	bool retval = false;
	...
	if (_i2cPort->endTransmission(false) != 0) 									// Send a restart command. Do not release bus.
  	{
    	lastCode = LPS33HW_CODE_ERR; 											// Sensor did not ACK
    	return false;
  	}
  	...
  	for(uint8_t indi = 0; indi < size; indi++)									// 
  	{
  		...
  		{
  			lastCode = LPS33HW_CODE_RX_UNDERFLOW; 								// For some reason we did not get as much data as we expected!
    		return false;
  		}
  	}		
...
/**
   ...
   * @param reg_adr The address of the first register to write to. Use of the LPS33HW_RegistersTypeDef enumeration values is suggested.
...
*/	
bool LPS33HW::write( uint8_t reg_adr, uint8_t * pdata, uint8_t size)
{
	_i2cPort->beginTransmission( sensor_address );		// Begin talking to the desired sensor
